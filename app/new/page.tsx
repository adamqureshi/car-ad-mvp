...
export default function NewAd() {
  ...
  // ----- submit (now saves to /api/ads and returns short id) -----
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      // convert to numbers for the API/store
      price: Number(onlyDigits(String(form.get("price") || "")) || 0),
      miles: Number(onlyDigits(String(form.get("miles") || "")) || 0),
      year: Number(onlyDigits(String(form.get("year") || "")) || 0),
      make: String(form.get("make") || "").trim(),
      model: String(form.get("model") || "").trim(),
      trim: String(form.get("trim") || "").trim(),
      // map notes -> description for now
      description: String(form.get("notes") || "").trim(),
      seller: {
        name: String(form.get("sellerName") || "").trim(),
        email: String(form.get("sellerEmail") || "").trim(),
        phone: sellerPhoneDigits,
      },
    };
    ...
    try {
      ...
      const { id } = await r.json();                 // short id
      const nextUrl = `${window.location.origin}/ad/${id}`;
      setUrl(nextUrl);

      // keep local “My ads” list
      const title =
    ...
    <div className="card">
      ...
      <form className="row" onSubmit={onSubmit} ref={formRef}>
        ...
        <a className="link" href="#" onClick={(e)=>{e.preventDefault(); (formRef.current as HTMLFormElement)?.requestSubmit();}}>
          {creating ? "Creating…" : "Save & get link"}
        </a>
      </form>
